...
def run_standalone_detection(camera_index=0):
    """獨立運行動作檢測（用於測試和調試）"""
    print("=== Running Boxing Detection in Standalone Mode ===")
    ...
    try:
        for frame_id, keypoints_data, frame in get_keypoints_stream(video_source=camera_index):
            # 鏡像翻轉 (注意：若攝影機本身就具有鏡像效果，即可刪除)
            frame = cv2.flip(frame, 1)

            # 檢測動作
            frame_data = detector.detect_actions(keypoints_data)

            # 可視化結果
            result_frame = visualizer.draw_debug_frame(frame, keypoints_data, frame_data)

            # 打印檢測到的動作（可選）
            for player in frame_data.players:
                if player.is_attacking:
                    print(f"Player {player.player_id}: {player.action_type} "
                          f"with {player.attack_hand} hand (conf: {player.confidence:.2f})")

            # 顯示結果
            cv2.imshow('Boxing Detection - Standalone', result_frame)
...
if __name__ == "__main__":
    ...
    # 運行獨立檢測模式
    run_standalone_detection(args.camera)
